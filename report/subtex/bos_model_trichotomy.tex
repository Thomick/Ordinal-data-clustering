...
\begin{lemma}[Log concavity affine times polynomial]
    ...
    Let $P$ be a strictly $\log$-concave function or without zeros on $I$ and $a, b \in \RR$ with $a \neq 0$ and $\forall t  \in I, at + b \neq 0$. Then $f: t \mapsto (at + b)P(t)$ is $\log$-concave on $I$.
...
\begin{proof}
    ...
    If $P$ is strictly $\log$-concave on $I$, using the lemma~\ref{lemma:concavity_log_composed_functions} we have that $P'(t)^2 - P(t)P''(t) > 0$.
    If $P$ is without zeros on $I$, we have that $\forall t \in I, P(t) > 0$. 

    As all the terms are $\geq 0$ and one of the two is strictly positive, hence we have that $\forall t \in I, f'(t)^2 - f(t) f''(t) > 0$ and using the lemma~\ref{lemma:concavity_log_composed_functions} we have that $f$ is strictly $\log$-concave on $I$.

    If we just have that $P$ is $\log$-concave on $I$ without the assumption on $a$ and $b$, we have that $\forall t \in I, f'(t)^2 - f(t) f''(t) \geq 0$ and using the lemma~\ref{lemma:concavity_log_composed_functions} we have that $f$ is $\log$-concave on $I$.
\end{proof}
...
\begin{lemma}[Log concavity of the BOS model]
    ...
    \[ \pi \mapsto \Pr(x | x \in e, e, \mu, \pi) \]
    If $\card{e} > 2$, this function is $\log$-concave on $[0, 1]$ and strictly $\log$-concave on $]0, 1[$. 
    If $\card{e} \leq 2$ this function is affine on $[0, 1]$ if $\card{e} \leq 2$.
\end{lemma}
\begin{proof}
    Let $m \in \NN^*$, $x \in \bbrack{1, m}$ and $\mu \in \bbrack{1, m}$.
    ...
    \begin{itemize}
        \item Initialization: $\card{e} \leq 2$:
        \[\pi \mapsto \Pr(x | x \in e, e, \mu, \pi) \] is a polynomial function of degree at most $1$ by lemma~\ref{lemma:bos_polynomial} and is therefore affine on $[0, 1]$.

        \item Induction: Suppose that $\card{e} > 2$ and that the result holds for all $f \subset \bbrack{1, m}$ of size less or equal than $\card{e} - 1$ and let us prove it for $\card{e}$.
        ...
        We have a sum of function. We now have to check that each function is $\log$-concave on $[0, 1]$ and at least one is strictly $\log$-concave on $]0, 1[$. We will use the lemma~\ref{lemma:concavity_log_polynomial_times_affine}. We first focus on $\Pr(f | x \in e, e, \mu, \pi)$.
        
        Using the lemma~\ref{lemma:bos_transition}, we have that  $\Pr(f | x \in e, e, \mu, \pi)$ is an affine function of $\pi$ times of the form $a_f \pi + b_f$ where $a_f = 1 - \frac{\card{f}}{\card{e}} \neq 0$ (as $\card{f} < \card{e}$) or $a_f = - \frac{\card{f}}{\card{e}} \neq 0$ and $b_f = \frac{\card{f}}{\card{e}}$.

        Let $t \in ]0, 1[$. We check that $a_f t + b_f \neq 0$. If $a_f = 1 - \frac{\card{f}}{\card{e}}$, we have that $a_f > 0, t > 0$ and $b_f > 0$ and therefore $a_f t + b_f > 0$. If $a_f = - \frac{\card{f}}{\card{e}}$, we have that $a_f t + b_f = \frac{\card{f}}{\card{e}} (1 - t) > 0$.
        
        Each $\pi \mapsto \Pr(x | x \in f, f, \mu, \pi)$ is $\log$-concave on $[0, 1]$ by induction hypothesis. Using the lemma~\ref{lemma:concavity_log_polynomial_times_affine}, we have that each $\pi \mapsto \Pr(x | x \in f, f, \mu, \pi) \Pr(f | x \in e, e, \mu, \pi)$ is $\log$-concave on $[0, 1]$. 
        
        We now need to check that at least one is strictly $\log$-concave on $]0, 1[$.
        ...
        \tr{Need to check why this does not work for |e| = 2}
    \end{itemize}
