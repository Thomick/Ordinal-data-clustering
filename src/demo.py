...
if __name__ == "__main__":
    ...
    if max_col[arg_max_all] > 150 and args.method == "bos":
        print(f"Two many categories for feature {arg_max_all}. The maximum number of categories for BOS model is 150 for this demo. Feature {arg_max_all} has {max_col[arg_max_all]} categories.")
        with open("demo_failure.txt", "w") as f:
            f.write(f"Two many categories for feature {arg_max_all}. The maximum number of categories for BOS model is 150 for this demo. Feature {arg_max_all} has {max_col[arg_max_all]} categories.")
        sys.exit(0)
    if max_col[arg_max_all] > 23 and args.method == "god":
        print(f"Two many categories for feature {arg_max_all}. The maximum number of categories for GOD model is 23 for this demo. Feature {arg_max_all} has {max_col[arg_max_all]} categories.")
        with open("demo_failure.txt", "w") as f:
            f.write(f"Two many categories for feature {arg_max_all}. The maximum number of categories for GOD model is 23 for this demo. Feature {arg_max_all} has {max_col[arg_max_all]} categories.")
        sys.exit(0)
    d = data.shape[1]
    m = n_cat
